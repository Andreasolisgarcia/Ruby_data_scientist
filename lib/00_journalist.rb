...
def totaljournalists
    total=@journalists.length
    puts "Combien y a-t-il de handle dans cette array ?"
    puts total
end

# Give the journalist with less characters

def smallerHandle
    smaller_handle=@journalists.sort_by(&:length)[0]
puts "Quelle est le handle le plus court de cette liste ?"
puts smaller_handle
end

# Number of Handles with 5 characters 
def fiveCharacters
    name_five_characters= 0
    @journalists.each do |journalist|
        number=journalist.length
        if  number == 6
            name_five_characters = name_five_characters + 1
        end
    end     
    puts "Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
    puts name_five_characters
end
#how many start with uppercase
def first_char_uppercase
    firstCharUppercase= 0
    @journalists.each do |journalist|
        char=journalist.slice(1)
        if  char==char.upcase
            firstCharUppercase = firstCharUppercase + 1
        end
    end     
    puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
    puts firstCharUppercase
end
#Journalists alphabetical order 
def alphabeticalOrder
    puts"Voici la liste triée par ordre alphabétique."
    puts @journalists.sort
end
#Journalists size  order 
def sizeOrder
    puts"Voici la liste triée par la longeur ( de plus petit au plus grand)."
...
#Where is @epenser
def findPosition
   # puts "qui tu veux trouver"
    #element=gets.chomp
    puts "Quelle est la position dans l'array de la personne @epenser ?"
    element="@epenser"
    index= @journalists.index(element)
    puts "il se trouve ici à la position #{index+1}"
end

#Journalists sepated by number of characters 
